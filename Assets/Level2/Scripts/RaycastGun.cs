...
public class RaycastGun : MonoBehaviour
{
    ...
    float fireTimer;
    int asteroidsDestroyed; // Nuevo: contador de asteroides destruidos
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Fire1") && fireTimer > fireRate)
        {
            ...
            if (Physics.Raycast(rayOring, playerCamera.transform.forward, out hit, gunRange))
            {
                ...
                if (hit.transform.tag == "asteroid") // Nuevo: si el objeto destruido es un asteroide, incrementar el contador
                {
                    asteroidsDestroyed++;
